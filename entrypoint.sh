...
if [ "$DISABLE_DISCOVERY" != "true" ]; then
    ...
    echo "[Entrypoint] Discovery script disabilitato (DISABLE_DISCOVERY=true)."
    if [ ! -f "$LITELLM_CONFIG_PATH" ]; then
        echo "[Entrypoint] Attenzione: $LITELLM_CONFIG_PATH non trovato. LiteLLM potrebbe fallire se il file non viene fornito esternamente."
    fi
fi

echo "[Entrypoint] Avvio LiteLLM Proxy..."
